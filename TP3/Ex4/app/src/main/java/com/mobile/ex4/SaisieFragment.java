...
public class SaisieFragment extends Fragment {
    ...
    public void Download(String _dlPath)
    {
        ResultReceiver _onDLFinished = new ResultReceiver(new Handler()) {
            @Override
            protected void onReceiveResult(int resultCode, Bundle resultData) {
                View _view = getView();
                if(_view == null) return;
                TextView _feedbackField = ((TextView)_view.findViewById(R.id.feedbackField));
                if(_feedbackField == null) return;
                _feedbackField.setText(resultData.getString("DLData"));
            }
        };

        Intent intent = new Intent(getActivity(), DownloadService.class);
        intent.putExtra("DLPath", _dlPath);
        intent.putExtra("OnDLFinishedReceiver", _onDLFinished);
    ...
    public void BindDownloadButton(View _view)
    {
        ((Button)_view.findViewById(R.id.buttonDownload)).setOnClickListener(new View.OnClickListener()
        {
            @Override
            public void onClick(View view) {
                Download("https://cat-fact.herokuapp.com/facts");
            }
        });
    }
